...
with col_map:
    ...
    # 運用滑走路に合わせて機首角を自動固定（RWY16なら右下、RWY34なら左上）
    ...
    # ▼ 修正：太陽位置の正確な計算と、「光が広がる面」のグラデーション構築
    sun_azimuth = 180 + (sim_hour - 12) * 15 # 方位角 (6時=90, 12時=180, 18時=270)
    sun_azimuth_rad = math.radians(sun_azimuth)
    
    # グラデーションが太陽の方向(0%)から反対側(100%)へ流れるように始点終点を計算
    ...
    <svg width="800" height="800" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
        <defs>
            <linearGradient id="sunLight" x1="{x1}%" y1="{y1}%" x2="{x2}%" y2="{y2}%">
                ...
                <stop offset="40%" stop-color="#FFD700" stop-opacity="0.1" />
    ...
    folium.Marker(
        plane_pos,
        tooltip=f"被写体 (方位角: {plane_heading}°) - タッチで移動",
        icon=folium.DivIcon(
            icon_size=(800, 800), # 巨大なSVGで面を表現
            icon_anchor=(400, 400), # 中心を合わせる
            html=plane_svg
        )
    ).add_to(m)
    ...
    if st.button("⚡ ブリーフィングをリクエスト", type="primary", use_container_width=True):
        with st.spinner("ANALYZING..."):
            try:
                ...
                prompt = f"福岡空港の撮影スポット「{spot_data['スポット']}」での空撮助言。現在時刻は{sim_hour}時。被写体の飛行機は現在地から見て機首を{plane_heading}度に向けています。この場所の特徴は「{spot_data['特徴']}」、マスターの持参する推奨焦点距離は「{spot_data['焦点距離']}」です。この機材と環境を活かしたマニアックな撮影戦術を解説せよ。Markdown記号は禁止。"
